...
def test_core_mvp_flow() -> None:
    ...
    session_resp = client.post(
        ...
        headers={"Idempotency-Key": "session-start-1"},
        json={
            "user_id": user_id,
            "plan_id": plan_id,
    ...
    complete_resp = client.post(f"/v1/workouts/sessions/{session_id}/complete")
    assert complete_resp.status_code == 200
    assert complete_resp.json()["status"] == "completed"

    recommendation_resp = client.post(
        f"/v1/users/{user_id}/recommendations",
        headers={"Idempotency-Key": "recommendation-1"},
    ...
    history_resp = client.get(f"/v1/workouts/history/{user_id}")

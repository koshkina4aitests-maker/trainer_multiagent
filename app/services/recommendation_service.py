...
class RecommendationService:
    ...
    def generate_for_user(self, user_id: int, current_condition: str, style: str) -> dict:
        ...
        items = self._build_recommended_items(latest_plan, style)
        if not items:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="No exercises available to build recommendation",
            )
    ...
    def _style_label(style: str) -> str:
        if style == "split_upper":
            return "split верх"
        if style == "split_lower":
            return "split низ"
        return "fullbody"

    def _build_recommended_items(self, latest_plan: WorkoutPlan, recommendation_style: str) -> list[dict]:
        all_items = sorted(latest_plan.items, key=lambda i: i.id)
        if not all_items:
            return []
        if recommendation_style == "fullbody":
            selected = all_items
        else:
            # Minimal deterministic split in absence of exercise taxonomy by upper/lower.
            midpoint = max(1, len(all_items) // 2)
            if recommendation_style == "split_upper":
                selected = all_items[:midpoint]
            else:
                selected = all_items[midpoint:]
                if not selected:
    ...
    def _build_rationale(
        ...
        latest_session: WorkoutSession | None,
        ...
        return " ".join(
            [
                ...
                f"Выбран стиль: {RecommendationService._style_label(recommendation_style)}.",
                safety_clause,
                readiness_clause,
                progression_clause,

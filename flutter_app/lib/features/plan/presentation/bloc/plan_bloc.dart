...
class PlanWorkoutUpdated extends PlanEvent {
  final PlannedWorkout workout;
  PlanWorkoutUpdated(this.workout);
}

class PlanRecommendationCopiedToPlan extends PlanEvent {
  final PlannedWorkout recommendation;
  final DateTime scheduledDate;
  PlanRecommendationCopiedToPlan(this.recommendation, this.scheduledDate);
}

class PlanRecommendationRequested extends PlanEvent {
  final RecommendationStyle? style;
  final ReadinessData? readiness;
  PlanRecommendationRequested({this.style, this.readiness});
}
...
class PlanBloc extends Bloc<PlanEvent, PlanState> {
  ...
  PlanBloc(this._repo) : super(PlanInitial()) {
    ...
    on<PlanWorkoutUpdated>(_onUpdate);
    on<PlanRecommendationCopiedToPlan>(_onCopyRecommendation);
  ...
  Future<void> _onUpdate(PlanWorkoutUpdated event, Emitter<PlanState> emit) async {
    await _repo.addPlannedWorkout(event.workout);
    final plan = await _repo.getWeekPlan(_currentWeekStart);
    final current = state;
    final rec = current is PlanLoaded ? current.recommendation : null;
  ...
  Future<void> _onCopyRecommendation(
    PlanRecommendationCopiedToPlan event,
    Emitter<PlanState> emit,
  ) async {
    final current = state;
    final rec = event.recommendation;
    await _repo.addPlannedWorkout(
      rec.copyWith(
        id: '',
    ...
    final updatedWeek = await _repo.getWeekPlan(_currentWeekStart);
    final recommendation = current is PlanLoaded ? current.recommendation : rec;
    emit(PlanLoaded(updatedWeek, _currentWeekStart, recommendation: recommendation));
  }
